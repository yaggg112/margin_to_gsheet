...
Sheet 結構：
  Sheet「每日維持率」（每天執行 append 一列，最新在最上方）：
    ...
    上市融資金額(億) | 上櫃融資金額(億) | 融資金額合計(億) | 大盤融資維持率(%)
...
SPREADSHEET_ID    = "1bWB2dmwiGXp9NJ9GCTHsRja6wkHz_TJFCE-rWftLdD8"  # Google Sheets 網址中的 ID
...
def _is_stock(code: str) -> bool:
    """純4碼數字 = 一般股票，其餘為 ETF/ETN/債券等"""
    return bool(re.match(r"^\d{4}$", str(code).strip()))


# ── 資料抓取（同 margin_today.py）────────────────────────────────────────────

def fetch_twse_data(date: str) -> tuple[float, float, pd.DataFrame, list[dict]]:
    """
    回傳：(上市融資金額元, 上市融資市值元, 個股明細df, ETF清單)
    """
    # 融資金額彙總
    ...
    etf_twse = [{"代號": d["證券代號"], "名稱": d["名稱"], "市場": "上市",
                 "備註": "非純4碼或含字母"}
                for d in all_records if not _is_stock(d["證券代號"])]

    margin_df = pd.DataFrame(
        [d for d in all_records if _is_stock(d["證券代號"])]
    )
    ...
    return twse_margin_amount, twse_value, merged, etf_twse
...
def fetch_tpex_data(date: str) -> tuple[float, float, list[dict]]:
    ...
    etf_tpex = [{"代號": d["證券代號"], "名稱": d["名稱"], "市場": "上櫃",
                 "備註": "非純4碼或含字母"}
                for d in all_records if not _is_stock(d["證券代號"])]
...
def get_data_auto(max_lookback: int = 10) -> dict:
    ...
    for i in range(max_lookback):
        ...
        try:
            twse_amount, twse_value, twse_df, etf_twse = fetch_twse_data(date)
            ...
            ratio = (total_value / total_amount * 100) if total_amount else None
...
def update_daily_sheet(ws, data: dict) -> None:
    """
    寫入每日維持率：最新資料插入第 2 列（第 1 列是欄位名稱）。
    若第 1 列是空的則先寫入欄位名稱。
    若該日期已存在則跳過（避免重複寫入）。
    """
    HEADERS_ROW = [
    ...
    # 插入第 2 列（index=2，gspread 從 1 開始）
...
def main():
    ...
    print(f"\n日期：{data['date']}")
    print(f"上市融資市值：{data['twse_value']:,.2f} 億  |  上市融資金額：{data['twse_amount']:,.2f} 億")
    print(f"上櫃融資市值：{data['tpex_value']:,.2f} 億  |  上櫃融資金額：{data['tpex_amount']:,.2f} 億")
    print(f"合計市值：{data['total_value']:,.2f} 億  |  合計金額：{data['total_amount']:,.2f} 億")
    print(f"大盤融資維持率：{data['ratio']} %\n")
